...
| D024 |  | data-model | AI 拆解对话持久化方案 | 新增 conversations + messages 两张表，完整持久化对话历史。模块数据嵌入 assistant 消息的 content 中，不新建草稿模块表。 | 刷新后需恢复完整上下文。AI 每次返回全量模块列表，数据就在消息里，无需额外同步。1:1 对话-里程碑关系简化管理。 | Yes | collaborative |
| D025 |  | frontend | 模块引用交互方式 | 显式引用（点击卡片引用按钮插入输入框）+ AI 自然语言推断（AI 从当前模块列表中查找）混合方案。 | 显式引用保证准确性（结构化数据随消息发送），AI 推断提升便利性（用户说"模块3"即可）。两者互补。 | Yes | collaborative |
| D026 |  | architecture | LLM 多轮对话输出格式 | LLM 每次返回完整模块列表，前端全量替换。不使用增量更新。 | 简单可靠，前端不需要合并逻辑。虽然 token 消耗更大，但拆解场景模块数量通常 <15，在可接受范围。 | Yes | collaborative |
| D027 |  | data-model | 提示词存储策略 | 全局默认提示词硬编码在后端（decompose-service 的 DEFAULT_SYSTEM_PROMPT），每个里程碑的自定义提示词存在 conversations.system_prompt 字段。不新建独立提示词表。 | 大部分里程碑用默认提示词，自定义是少数场景。存在 conversation 上而非 milestone 上，因为提示词是对话的一部分——重新拆解会创建新对话。 | Yes | collaborative |
| D028 |  | frontend | MdViewer 目录导航使用 scrollTop 替代 scrollIntoView | 点击目录项时使用内容容器的 scrollTop 而非 scrollIntoView，确保只有内容区域滚动，外部布局不受影响。 | scrollIntoView 会滚动所有可滚动的祖先容器，导致外部页面也滚动，目录被滚出视口。scrollTop 只操作目标容器，行为可控。 | Yes | agent |
